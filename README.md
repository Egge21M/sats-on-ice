...
Local `setup` and `verify` use Coco's explicit `Manager` initialization path: `initializeCoco()` in the pinned version runs operation recovery even when watchers are disabled. `serve` validates wallet state and registers the configured mint through a temporary local manager, then uses `initializeCoco()` for the active wallet and startup recovery. Workers start after the factory returns so their startup failures can be cleaned up before retrying; see the [receiving lifecycle](docs/design/lightning-receiving.md#implementation). Coco is disposed before the caller-owned SQLite connection is closed.
