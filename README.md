...
Local `setup` and `verify` use Coco's explicit `Manager` initialization path: `initializeCoco()` in the pinned version runs operation recovery even when watchers are disabled. `serve` validates wallet state and registers the configured mint through a temporary local manager, then uses `initializeCoco()` with its default watchers, processors and startup recovery. Application payout reactions use `Manager.on`. Returned managers are disposed before the caller-owned SQLite connection is closed; see the [receiving lifecycle](docs/design/lightning-receiving.md#implementation) for the factory failure-cleanup limitation.
