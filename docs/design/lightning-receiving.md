...
`src/receiving-wallet.ts` keeps active wallet startup behind its existing `createInvoice`/`close` interface. A temporary local manager refuses other-mint wallet state and registers/trusts the configured mint before any recovery, then is disposed. `initializeCoco()` constructs the active manager, initializes the fee plugin, reconciles legacy quotes and runs Coco's startup recovery, including mint and melt operations. Setup/verify retain explicit local `Manager` construction and remain observational with respect to payments, as required by [ADR 0009](../adr/0009-local-cli-without-payment-recovery.md).
...
The startup regression check also rejects a changed configured mint before claiming a paid invoice. Factory failure cleanup is subject to the dependency limitation described above.
