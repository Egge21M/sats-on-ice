...
Selected topology: one Machine with one mounted volume, containing the complete SQLite database, including configuration, seed, proofs, and payout index. `/data/sats-on-ice.sqlite` is a possible path, not an existing configuration. Replicating Machines without a wallet/database coordination design would create independent or stale wallet state, not transparent failover.
...
`fly ssh console --machine <id> -C '<application CLI command>'` executes a command on an existing running Machine and therefore can reach its mounted database. This fits the chosen CLI configuration model; restart the application after permitted username/threshold changes. [SSH command reference](https://fly.io/docs/flyctl/ssh-console/).

Open bootstrap detail: if an unconfigured server immediately exits, ordinary SSH setup cannot rely on it staying running. Define a setup command or setup mode on the volume-attached Machine before serving payments. Build-time seed/configuration generation would target the image filesystem, not this persistent database.
