...
`verify` opens an existing database and validates its setup; it refuses a missing database. Both commands may apply database migrations, so local inspection is not a read-only SQLite connection. [src/setup.ts](../../src/setup.ts) initializes `SqliteRepositories` and reads local wallet state directly without constructing a manager, invoking startup recovery or starting watchers or processors, as recorded in [ADR 0009](../adr/0009-local-cli-without-payment-recovery.md).

Inspection checks Coco's mint repository and all ready proofs, including reserved proofs and every unit, and rejects a different mint. The server shares this repository check before initializing its active manager. It sums `proofRepository.getAvailableProofs(config.mintUrl, { unit: "sat" })` using bigint arithmetic, excluding reserved, inflight, spent and non-sat proofs. Setup persists the configured mint URL without contacting or registering the mint in Coco; capability checks and active mint initialization belong to the server slice. The application closes its SQLite connection after inspection, including error paths; there is no local manager lifecycle to dispose.
