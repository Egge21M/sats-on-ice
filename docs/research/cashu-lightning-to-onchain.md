...
Official adapter documentation explicitly supports `@cashu/coco-sqlite-bun`, exporting `SqliteRepositories` around a `bun:sqlite` `Database`, supplied to `initializeCoco`. The separate Node adapter uses `better-sqlite3`. This supports Bun + TypeScript + SQLite as a stack recommendation without selecting an HTTP framework. Exact core/adapter versions and their behavior under process termination still require installation and integration verification. [Storage adapters](https://cashubtc.github.io/coco/pages/storage-adapters.html).
...
- Pin and verify the chosen Coco release before implementation. Retrieved official pages contained both old `prepare({ mintUrl, ... })` examples and newer quote-first signatures; the current stable operation reference and published package README agree on quote-first preparation. This research did not compile those calls or establish one exact compatible release set. [Older minting guide snapshot](https://cashubtc.github.io/coco/starting/minting.html), [current mint operations](https://cashubtc.github.io/coco/pages/mint-operations.html), [published core package](https://www.npmjs.com/package/@cashu/coco-core).
