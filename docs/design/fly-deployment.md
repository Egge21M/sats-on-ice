...
Fly checks `GET /readyz` every 30 seconds after a 30-second startup grace period. It returns only `{ "ready": true }` with HTTP 200 once capability checks, Coco initialization and payment reconciliation succeed, otherwise `{ "ready": false }` with HTTP 503. The route has no identity, balances, seed or diagnostics, creates no invoice and ordinarily reports recorded state without contacting the mint; a request after a detected pause can initiate reconciliation. All other methods return 405. Local `status` and `fly logs --app "$SOI_APP"` provide the more detailed safe diagnostic messages.

Readiness is the server's recorded capability and reconciliation state, with a last-successful-reconciliation timestamp in CLI status. It **does not prove continuous connectivity, processor health or freshness of the CLI's SQLite snapshot**. A detected process pause makes receiving and new sweeps unready until the same manager reconciles. See the [payment resume guide](payment-resume.md) for behavior, limits and controlled warm-resume evidence. Coco 2.0.0 can leave workers behind if its factory throws after starting them without returning a manager; retries after such failures may require a process restart. Returned managers are disposed on shutdown.
...
This exercises the production image, public TLS/Host behavior, automatic bootstrap, local CLI and persistent state through cold restart and image deployment. It does not demonstrate Lightning routing, Bitcoin broadcast/confirmation, or warm-resume payment correctness. The separate [resume check](payment-resume.md#controlled-fly-check) covers payment while suspended; its local integration tests also cover stale connections and interruption during payout submission.
...
Both disposable apps and their test volume were destroyed after verification; no deployment from this run remains. No real Lightning payment or Bitcoin broadcast occurred. This earlier deployment run did not verify warm-resume reconciliation; the subsequent [payment resume check](payment-resume.md#controlled-fly-check) covers it.
